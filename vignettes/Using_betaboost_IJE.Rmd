...
We can also use look at the fitted values or predict new observations 

```{r}
# fitted values
preds <- predict(beta4, type = "response")
summary(cbind(preds$mu, preds$ph))
```


```{r}
# predictions for two new obs from the two treatment arms
predict(beta3, newdata = data.frame(pain = c(0.3, 0.3), arm = c(0,.01)), 
                 type = "response")
```
