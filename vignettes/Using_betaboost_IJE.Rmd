...
The raw outcome is distributed as follows (histogram of the Quality of Life with smooth kernel density estimate).
```{r outcome}
...
The influence of various predictors on the outcome is depicted in the following: First, boxplots comparing the Quality of Life outcome for both treatment arms. Second, a scatterplot for the relationship between the explanatory variable `pain` (x-axis) and the outcome variable (y-axis).
...
nuisance(beta1)
```
Note, that here we automatically 100 boosting iterations `mstop = 100`, as this is the default value. 
